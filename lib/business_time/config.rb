...
module BusinessTime
  ...
  class Config
    ...
    threadsafe_cattr_accessor :holidays
		...
		# You can set weekend off, to make weekend behave as work days.
    threadsafe_cattr_accessor :weekends_off
    ...
    class << self
      ...
      def load(file)
        ...
        (config["weekends_off"] || []).each do |weekend|
          weekends_off << Date.parse(weekend)
        end
      end
